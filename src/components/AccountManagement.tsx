...
export default function AccountManagement({ onClose, isDark }: AccountManagementProps) {
  ...
  const [updatesSubView, setUpdatesSubView] = useState<'grid' | 'link_cadastro' | 'aniversarios' | 'treinos' | 'avaliacao_fisica' | 'planos_recorrentes'>('grid');
                  ...
                  <div className="bg-[#0c1622] px-5 pt-6 pb-4 flex flex-col gap-1 shrink-0">
                    ...
                    <h4 className="text-white text-xl font-black italic uppercase tracking-tighter">
                      {updatesSubView === 'link_cadastro' ? 'Link de Cadastro' :
                       (updatesSubView === 'aniversarios' || updatesSubView === 'treinos' || updatesSubView === 'avaliacao_fisica' || updatesSubView === 'planos_recorrentes') ? 'Atualizações' :
                       'Atualizações'}
                    </h4>
                  </div>
                ) : (
